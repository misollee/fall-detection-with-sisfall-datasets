...
# summary by people, activities ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

p = person_lis[0]
total_df = pd.DataFrame()
for p in person_lis:
    person_df = pd.read_csv('data/feature_extract/'+p+'.csv', index_col = 0)
    features = np.setdiff1d(person_df.columns,['window','id','trial', 'activity'])
    person = person_df.groupby(['id','trial','activity'])[features].max()
    person.reset_index(inplace=True)
    total_df = pd.concat([total_df, person])

total_df.to_csv('data/total_df_ver2.csv')
